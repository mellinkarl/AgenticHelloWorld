...
def ia_node(state: GraphState) -> Dict[str, Any]:
    ...
    if not (isinstance(uri, str) and uri.startswith("gs://")):
        err = "IA: Missing or invalid GCS URI"
        print(f"[IA][ERR] {err}")
        messages.append(err)
        return {
            "runtime": {"ia": {"status": "FAILED", "route": []}},
            "artifacts": {"ia": {"doc_gcs_uri": uri}},
            "internals": {},
            "message": messages,
        }
    ...
    try:
        ...
        messages.append(f"download ok={dl.get('ok')} size={dl.get('size')} ct={dl.get('content_type')}")
    except Exception as e:
        err = f"IA: exception during download: {e}"
        print(f"[IA][ERR] {err}")
        messages.append(err)
        return {
            "runtime": {"ia": {"status": "FAILED", "route": []}},
    ...
    if not dl["ok"] or not isinstance(dl.get("data"), (bytes, bytearray)):
        err = dl.get("error") or "unknown download error"
        print(f"[IA][ERR] {err}")
        messages.append(err)
        return {
            "runtime": {"ia": {"status": "FAILED", "route": []}},
    ...
    if ext == ".pdf" or ct.lower() == "application/pdf" or obj_name.lower().endswith(".pdf"):
        try:
            ...
            messages.append("PDF validation ok")
        except Exception:
            err = "IA: file not valid PDF"
            print(f"[IA][ERR] {err}")
            messages.append(err)
            return {
                "runtime": {"ia": {"status": "FAILED", "route": []}},
    ...
    try:
        ...
        print(f"[IA] wrote to {file_uri}")
        messages.append(f"wrote to {file_uri}")
    except Exception as e:
        err = f"IA: failed to write local file: {e}"
        print(f"[IA][ERR] {err}")
        messages.append(err)
        return {
            "runtime": {"ia": {"status": "FAILED", "route": []}},
    ...
    message_summary = f"IA: success | local_uri={file_uri}"
    messages.append(message_summary)
    print(f"[IA] {message_summary}")
